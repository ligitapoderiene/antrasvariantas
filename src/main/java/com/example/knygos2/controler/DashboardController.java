...
public class DashboardController implements Initializable {
    ...
    public void onCreateButtonClick() {
        ...
        if (!Validation.isValidTitle(titleField2)) {
            ...
            // Kuriame įrašą DB
            Book book = new Book(titleField2, pageField3, category, autor, summary2);
            BookDao.create(book);
    ...
    public void onUpdateButtonClick() {
        String titleField2 = titleField.getText();
        String secondsField2 = pageField.getText();          // Paliekam String tipa kad veiktu validacija
        ...
        String year = "";
        if (!choiceBoxSummary.getSelectionModel().isEmpty()) {
            year = choiceBoxSummary.getSelectionModel().getSelectedItem().toString();
        }
        ...
        if (!Validation.isValidTime(idField2)) {
            ...
            status.setText("Neteisingai įvedėte pavadinimą");
        } else if (!Validation.isValidTime(pageField4)) {
            status.setText("Neteisingai įvedėte lapu skaiciu");
        } else if (category.isEmpty()) {
            status.setText("Prašome pasirinkti kategorija");
        } else {
            ...
            // Kuriame įrašą DB
            Book book = new Book(idField2, titleField2, pageField4, category, autor, summary3);
            BookDao.update(book);
